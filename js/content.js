...
Gallerify.prototype.run = function () {
	...
	// Add gallery section.
	self.gallery = $('<section class="gallerified"></section>').hide()
		.insertAfter(document.head)
		.fadeIn(self.config.transitionSpeed, function () {
			$(document.body).hide(); })

	var container = $('<div>').appendTo(self.gallery)
	...
	$.each(self.items, function () {
		self.addImage.call(this, self, container);
	});
};

Gallerify.prototype.addImage = function(self, container) {
	...
	img.on('load', function () {
		...
		if (self.config.freezeGifs && is_gif_image(img[0])) {
			freeze_gif(img[0]);
...
Gallerify.prototype.hideLoading = function() {
  if (this.items && this.loaded == this.items.length) {
  	this.loading.css('opacity', 0).delay(600).slideUp(400);
  }
};
...
Gallerify.prototype.restore = function () {
	...
	self.gallery.fadeOut(self.config.transitionSpeed, function () {
		$(this).remove();
	});
};
